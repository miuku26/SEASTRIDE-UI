...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [dailyCoinsHistory, setDailyCoinsHistory] = useState<DailyCoinRecord[]>(INITIAL_COIN_RECORDS);
  ...
  const earnCoins = (amount: number) => {
    ...
    setDailyCoinsHistory(prev => {
      ...
      if (todayIndex >= 0) {
        next[todayIndex] = { ...next[todayIndex], coins: next[todayIndex].coins + amount };
      }
      return next;
    });
  };
